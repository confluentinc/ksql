def config = {
    ...
    dockerImageNames = ['confluentinc/ksqldb-cli', 'confluentinc/ksqldb-server']
...
def defaultParams = [
        booleanParam(name: 'RELEASE_BUILD',
                ...
                defaultValue: false),
        // Rename this it overlaps with environment variable i think
...
def job = {
            ...
            withCredentials([
                ...
                withDockerServer([uri: dockerHost()]) {
                    withMaven(globalMavenSettingsConfig: 'jenkins-maven-global-settings', options: [findbugsPublisher(disabled: true)]) {
                        ...
                        cmd += "-Ddocker.upstream-tag=master-latest "
    ...
    if (true || !config.isPrJob) {
        stage('Publish Docker Images') {
            withDockerServer([uri: dockerHost()]) {
                config.dockerRepos.eachWithIndex { dockerRepo, index ->
                    imageName = config.dockerImageNames[index]
                    // rename the docker image to use the db postfixed image name
                    sh "docker tag ${config.dockerRegistry}${dockerRepo}:${config.docker_tag} ${config.dockerRegistry}${imageName}:${config.docker_tag}"
                    // re-tag it to also specify that this is master-latest
                    sh "docker tag ${config.dockerRegistry}${dockerRepo}:${config.docker_tag} ${config.dockerRegistry}${imageName}:master-latest"

                    // publish both images
                    sh "docker push ${config.dockerRegistry}${imageName}:${config.docker_tag}"
                    sh "docker push ${config.dockerRegistry}${imageName}:master-latest"
