...
* ``CREATE STREAM AS SELECT ...`` and ``CREATE TABLE AS SELECT ...`` statements will use the source
topic partitions and replica counts for the sink topic if the config properties are not set in ``WITH`` clause.
In case of JOIN, the left hand side topic will be used.
