...
of reading input or config properties such as producer buffer size. In order to make the composition of output for the test cases
...
result topic before processing the next input message. This means that for the queires running in the testing tool, we have the same behavior as
 ...
 real executions because of buffering, while in the testing tool execution we will create every possible intermediate results.


Kafka Cluster
-------------

The KSQL testing tool does not use a real Kafka cluster and simulates the behavior of a cluster with single broker for the KSQL queries.
This means that the testing tool ignores configurations such as number of partitions or replicas for the input and output topics.
...
only the statements before it can potentially affect it's results. This is a different behavior than KSQ cluster where statements that
are submitted later can affect the output of a query. As an example, consider the following set of statements:
...
If you run the above statements in a real KSQL cluster, you will see one result generated for each INSERT INTO statements (You will have 5 messages in the output).
On the other hand, if you run the above statements in the testing tool, only the INSERT INTO statements before the CSAS query will generate results and the testing tool won't
run the query for the messages generated by the INSERT INTO statements after the CSAS statement.

Another important processing order to keep in mind for the testing tool is the order in which the input data for a query is processed. For a given query,
the testing tool will first process the input messages provided in the input file. After fully processing such messages
the testing tool then inspects the source topics for the query in the simulated Kafka cluster and processes any messages in those topics.
For the JOIN queries where we have more than one source topic, the testing tool first processes the left hand side topic and
