...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Roles and permissions to start KSQL Server
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The roles mentioned in the Access to KSQL are a minimal set of roles and resources required for a user to connect to KSQL and execute KSQL commands, including transient queries. However, any command that attempts to read, write, list, and create topics require the user to have necessary permissions in Kafka and/or Schema Registry to be able to proceed with the command execution.



To start KSQL Server and connect to a secured Kafka cluster, a user must have
the following roles granted by an administrator with the ``UserAdmin`` role.

.. note::

   In the following examples, the KSQL_CLUSTER environment variable is set to
   the KSQL service ID.

Roles on the KSQL cluster scope
  ``ResourceOwner`` role on the KSQL cluster, which binds ownership of the
  KSQL cluster's resources to the user. 

  ::

     KsqlCluster:LITERAL:ksql-cluster

Roles on the Kafka cluster scope
  ``ResourceOwner`` role on the KSQL command topic, the processing log topic, 
  and the consumer group.

  ::

     Topic:LITERAL:_confluent-ksql-${KSQL_CLUSTER}_command_topic
     Topic:LITERAL:${KSQL_CLUSTER}ksql_processing_log
     Group:PREFIXED:_confluent-ksql-${KSQL_CLUSTER}

^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Roles and permissions to contribute to KSQL
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To access the KSQL cluster for development, a user must have the following
roles granted by an administrator with the ``UserAdmin`` role.

Roles on the KSQL cluster scope
  ``DeveloperWrite`` role on the KSQL cluster  
  ::

     KsqlCluster:LITERAL:ksql-cluster

Roles on the Kafka cluster scope
  ``DeveloperRead`` role on the internal topics generated by KSQL for
  transient queries, joins, and other operations.

  ::

     Group:PREFIXED:_confluent-ksql-${KSQL_CLUSTER}transient

^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Roles and permissions to terminate a KSQL cluster
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To terminate a KSQL cluster by calling the REST /ksql/terminate API, a user must
have the following roles granted by an administrator with the ``UserAdmin`` role.

Roles on the KSQL cluster scope
  ``DeveloperManage`` role on the KSQL cluster.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Grant users access to resources
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

.. important::

   The roles that the user has must be granted to the KSQL server user as well.
   The KSQL server executes the same user commands in the background and by
   other servers connected to the same KSQL cluster.

Read from a topic
  - ``DeveloperRead`` or ``ResourceOwner`` role on the Kafka topic
  - ``DeveloperRead`` or ``ResourceOwner`` role on the |sr| subject
  These roles enable a user to list and read the topics from the
  SHOW TOPIC and PRINT <topic> statements. Also, the user can CREATE streams
  and tables from these topics, as well as execute SELECT statements on their
  streams and tables. If a SELECT statement contains a JOIN that uses an
  unauthorized topic, the SELECT fails with an authorization error.

Write to a topic
  - ``DeveloperWrite`` or ``ResourceOwner`` role on the Kafka topic
  - ``DeveloperWrite`` or ``ResourceOwner`` role on the Schema Registry subject
...
Create a stream with a persistent query
  - ``ResourceOwner`` role on the KSQL sink topic
  - ``ResourceOwner`` role on the |SR| sink subject
  These roles enable a user to create streams and tables with persistent
  queries. Because KSQL creates a new topic and a new SR schema, the user must
  have enough permissions to create, read, and write to the topic and |sr|
  subject.

Delete a topic
  - ``DeveloperManage`` or ``ResourceOwner`` role on the Kafka topic
  - ``DeveloperManage`` or ``ResourceOwner`` role on the |sr| subject
  These roles enable a user to delete a topic by using the DROP STREAM and
  DELETE TOPIC statements.

^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
RBAC workflow for developing a new KSQL application
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
