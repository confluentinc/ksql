...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Assign roles to manage KSQL access 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The following section defines the minimal set of roles and resources required
for a user to connect to KSQL and execute KSQL statements, including transient
queries.

.. important::

   Further access to resources on the KSQL and Kafka clusters must be granted 
   explicitly to the user by a principal with the ``UserAdmin`` role. Any KSQL
   statement that reads, writes, lists, and creates topics requires that the
   user has necessary permissions in Kafka and |sr|. For more information, see
   :ref:`config-security-ksql-grant-access`.

In the following examples, the KSQL_CLUSTER environment variable is set to the
KSQL service ID.
...
.. _config-security-ksql-grant-access:
