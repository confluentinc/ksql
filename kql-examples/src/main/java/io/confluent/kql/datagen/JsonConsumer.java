...
//        });
        ...
        KTable<String, GenericRow> source = builder.table(Serdes.String(), getGenericRowSerde(), topicName, "users");
    ...
    private KStream<String, GenericRow> orderFilter(KStream<String, GenericRow> orderStream) {
        ...
        KStream<String, GenericRow> orderFilter = orderStream.filter(new Predicate<String, GenericRow>() {
            @Override
    ...
    private  KStream<String, GenericRow> orderProject(KStream<String, GenericRow> orderStream) {
        ...
        KStream<String, GenericRow> orderProject = orderStream.map(new KeyValueMapper<String, GenericRow, KeyValue<String,GenericRow>>() {
            @Override
            public KeyValue<String, GenericRow> apply(String key, GenericRow value) {
                List<Object> newColumns = Arrays.asList(value.getColumns().get(0), value.getColumns().get(1), value.getColumns().get(3));
    ...
    private KTable<Windowed<String>, Long> orderUnitsPer10Seconds(KStream<String, GenericRow> sourceOrderStream) {
    ...
    public void joinTest() {
        ...
        KStream<String, GenericRow> pageviewStream = builder.stream(Serdes.String(), getGenericRowSerde(), "streams-pageview-input");
...
//        });
        ...
        KTable<String, GenericRow> usersTable = builder.table(Serdes.String(), getGenericRowSerde(), "streams-userprofile-input", "users");

        usersTable.mapValues(new ValueMapper<GenericRow, GenericRow>() {
            @Override
            public GenericRow apply(GenericRow genericRow) {
                System.out.println(genericRow.toString());
                return genericRow;
