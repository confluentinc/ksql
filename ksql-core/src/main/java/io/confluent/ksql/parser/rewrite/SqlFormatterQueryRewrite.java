            ...
            extends AstVisitor<Void, Integer>
    {
        ...
        protected Void visitQuery(Query node, Integer indent)
        {
            ...
            if (!node.getOrderBy().isEmpty()) {
                append(indent, "ORDER BY " + ExpressionFormatterQueryRewrite.formatSortItems(node.getOrderBy()))
                        .append('\n');
            }
        ...
        protected Void visitQuerySpecification(QuerySpecification node, Integer indent)
        {
            ...
            if (node.getWhere().isPresent()) {
                append(indent, "WHERE " + ExpressionFormatterQueryRewrite.formatExpression(node.getWhere().get()))
                        .append('\n');
            }

            if (node.getGroupBy().isPresent()) {
                append(indent, "GROUP BY " + (node.getGroupBy().get().isDistinct() ? " DISTINCT " : "") + ExpressionFormatterQueryRewrite.formatGroupBy(node.getGroupBy().get().getGroupingElements())).append('\n');
            }

            if (node.getHaving().isPresent()) {
                append(indent, "HAVING " + ExpressionFormatterQueryRewrite.formatExpression(node.getHaving().get()))
                        .append('\n');
            }
...
//            }
        ...
        protected Void visitShowCatalogs(ShowCatalogs node, Integer context)
        {
            ...
            node.getLikePattern().ifPresent((value) ->
                    builder.append(" LIKE ")
                            .append(ExpressionFormatterQueryRewrite.formatStringLiteral(value)));
        ...
        protected Void visitCreateTableAsSelect(CreateTableAsSelect node, Integer indent)
        {
            ...
            if (!node.getProperties().isEmpty()) {
                ...
                Joiner.on(", ").appendTo(builder, transform(node.getProperties().entrySet(),
                        entry -> entry.getKey() + " = " + ExpressionFormatterQueryRewrite.formatExpression(entry.getValue())));
                builder.append(")");
            }
