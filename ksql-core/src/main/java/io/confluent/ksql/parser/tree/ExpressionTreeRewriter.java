    ...
    public Expression visitFunctionCall(FunctionCall node, Context<C> context) {
      ...
      if (node.getWindow().isPresent()) {
          ...
          if (rewrittenEnd.isPresent()) {
            ...
            if (value.isPresent()) {
              ...
              if (rewrittenValue != value.get()) {
                rewrittenEnd =
                    Optional.of(new FrameBound(rewrittenEnd.get().getType(), rewrittenValue));
              }
            }
          }
        ...
        if (!sameElements(window.getPartitionBy(), partitionBy.build()) ||
          ...
          rewrittenWindow =
              Optional.of(new Window(partitionBy.build(), orderBy.build(), rewrittenFrame));
        }
      }
      ...
      if (!sameElements(node.getArguments(), arguments.build()) || !sameElements(rewrittenWindow,
                                                                                 node.getWindow())) {
        return new FunctionCall(node.getName(), rewrittenWindow, node.isDistinct(),
                                arguments.build());
      }
