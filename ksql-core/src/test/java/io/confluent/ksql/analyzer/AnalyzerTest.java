...
public class AnalyzerTest {
    ...
    public void testSimpleQueryAnalysis() throws Exception {
        ...
        Assert.assertTrue("INTO was not analyzed correctly.", analysis.into.getName().equalsIgnoreCase("testOutput"));
        Assert.assertTrue("FROM was not analyzed correctly.", analysis.fromDataSources.get(0).getLeft().getName().equalsIgnoreCase("test1"));
        Assert.assertTrue(analysis.getSelectExpressions().size() == analysis.getSelectExpressionAlias().size());
        String sqlStr = SqlFormatterQueryRewrite.formatSql(analysis.getWhereExpression()).replace("\n", " ");
        Assert.assertTrue(sqlStr.equalsIgnoreCase("(TEST1.COL0 > 100)"));

        String select1 = SqlFormatterQueryRewrite.formatSql(analysis.getSelectExpressions().get(0)).replace("\n", " ");
        Assert.assertTrue(select1.equalsIgnoreCase("TEST1.COL0"));
        String select2 = SqlFormatterQueryRewrite.formatSql(analysis.getSelectExpressions().get(1)).replace("\n", " ");
    ...
    public void testSimpleLeftJoinAnalysis() throws Exception {
        ...
        Assert.assertNotNull("SELECT aliacs is null", analysis.selectExpressionAlias);
        Assert.assertTrue("INTO was not analyzed correctly.", analysis.into.getName().equalsIgnoreCase("testOutput"));
        Assert.assertTrue("JOIN left hand side was not analyzed correctly.", analysis.join.getLeftAlias().equalsIgnoreCase("t1"));
        Assert.assertTrue("JOIN right hand side was not analyzed correctly.", analysis.join.getRightAlias().equalsIgnoreCase("t2"));
