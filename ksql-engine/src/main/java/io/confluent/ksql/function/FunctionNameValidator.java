...
class FunctionNameValidator implements Predicate<String> {
  ...
  private static final Set<String> KSQL_RESERVED_WORDS
      = ImmutableSet.<String>builder()
      .add("analyze").add("as").add("catalog").add("create").add("beginning").add("by")
      .add("delete").add("describe").add("drop").add("exists").add("explain").add("export")
      .add("extended").add("insert").add("into").add("join").add("from").add("list").add("load")
      .add("limit").add("not").add("partition").add("print").add("properties").add("queries")
      .add("registered").add("run").add("script").add("select").add("set").add("show")
      .add("stream").add("streams").add("table").add("tables").add("terminate")
      .add("topics").add("unset").add("when").add("where").add("window").add("with")
      .build();
