...
public class Fetcher<K, V> implements SubscriptionState.Listener, Closeable {
  ...
  public Map<TopicPartition, List<ConsumerRecord<K, V>>> fetchedRecords() {
    ...
    try {
      while (recordsRemaining > 0) {
        if (nextInLineRecords == null || nextInLineRecords.isFetched) {
          ...
          if (completedFetch.partitionData.records.sizeInBytes() > 0) System.out.println(Thread.currentThread().getName() + " parse record....:" + completedFetch + " t:" + completedFetch.partition + " offset:" + completedFetch.fetchedOffset + " dataBytes:" + completedFetch.partitionData.records.sizeInBytes() );
    ...
    if (fetched.size() > 0) {
      System.out.println(">>>");
      for (Map.Entry<TopicPartition, List<ConsumerRecord<K, V>>> topicPartitionListEntry : fetched.entrySet()) {
        System.out.println(new Date().toString() + " Fetched:" + topicPartitionListEntry.getKey() + " :" + topicPartitionListEntry.getValue().size());
      }
      System.out.println("<<<<");
    }
    return fetched;
  ...
  private Map<Node, FetchRequest.Builder> createFetchRequests() {
    ...
    System.out.println(Thread.currentThread().getName() + "fetchablePartitioins:::::::" + fetchablePartitions());
