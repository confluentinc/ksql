...
public class AnalyzerTest {
  ...
  public void init() {
    ...
    avroMetaStore = MetaStoreFixture.getNewMetaStore(new InternalFunctionRegistry(),
  ...
  public void shouldUseExplicitNamespaceForAvroSchema() {
    ...
    final Table intoTable = new Table(QualifiedName.of(createStreamAsSelect.getName().toString()));
    intoTable.setProperties(createStreamAsSelect.getProperties());
    final QuerySpecification querySpecification = (QuerySpecification) createStreamAsSelect
        .getQuery().getQueryBody();
    final QuerySpecification newQuerySpecification = new QuerySpecification(
        querySpecification.getSelect(),
        intoTable,
        true,
    ...
    assertThat(analysis.getIntoProperties().get(DdlConfig.VALUE_AVRO_SCHEMA_FULL_NAME).toString(),
        equalTo("'com.custom.schema'"));
  }
  ...
  public void shouldFailIfExplicitNamespaceIsProvidedForNonAvroTopic() {
    ...
    expectedException
        .expectMessage(DdlConfig.VALUE_AVRO_SCHEMA_FULL_NAME + " is only valid for AVRO topics.");
    analyzer.visitQuerySpecification(newQuerySpecification, new AnalysisContext(null));
  }
