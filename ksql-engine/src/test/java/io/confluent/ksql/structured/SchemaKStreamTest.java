...
public class SchemaKStreamTest {
  ...
  public void init() {
    ...
    rightSerde = getRowSerde(secondKsqlStream.getKsqlTopic(), secondKsqlStream.getSchema());
  ...
  @Test
  public void shouldSelectSchemaKStream() {
    // Given:
    ...
    // Then:
    final Schema resultSchema = result.getSchema();
    assertThat(resultSchema.fields(), hasSize(3));
    assertThat(resultSchema.fields().get(0).name(), is("COL0"));
    assertThat(resultSchema.fields().get(0).schema().type(), is(Schema.Type.INT64));
    assertThat(resultSchema.fields().get(1).name(), is("COL2"));
    assertThat(resultSchema.fields().get(1).schema().type(), is(Schema.Type.STRING));
    assertThat(resultSchema.fields().get(2).name(), is("COL3"));
    assertThat(resultSchema.fields().get(2).schema().type(), is(Schema.Type.FLOAT64));
    assertThat(result.getSourceSchemaKStreams().get(0), is(sameInstance(initialSchemaKStream)));
  }
  ...
  public void shouldUpdateKeyIfRenamed() {
    ...
    // When:
    final SchemaKStream projectedSchemaKStream = initialSchemaKStream
        .select(selectExpressions, childContextStacker, processingLogContext);
  ...
  @Test
  public void shouldSelectWithExpression() {
    // Given:
    final PlanNode logicalPlan = givenInitialKStreamOf(
        "SELECT col0, LEN(UCASE(col2)), col3*3+5 FROM test1 WHERE col0 > 100;");
    final ProjectNode projectNode = (ProjectNode) logicalPlan.getSources().get(0);
    final List<SelectExpression> projections = projectNode.getProjectSelectExpressions();

    // When:
    final SchemaKStream result = initialSchemaKStream
        .select(projections, childContextStacker, processingLogContext);;
    ...
    assertThat(resultSchema.fields().get(0).name(), is("COL0"));
    assertThat(resultSchema.fields().get(0).schema().type(), is(Schema.Type.INT64));
    assertThat(resultSchema.fields().get(1).name(), is("KSQL_COL_1"));
    assertThat(resultSchema.fields().get(1).schema().type(), is(Schema.Type.INT32));
    assertThat(resultSchema.fields().get(2).name(), is("KSQL_COL_2"));
    assertThat(resultSchema.fields().get(2).schema().type(), is(Schema.Type.FLOAT64));
    assertThat(result.getSourceSchemaKStreams().get(0), is(sameInstance(initialSchemaKStream)));
  }
  ...
  public void shouldFilter() {
    ...
    // When:
    final SchemaKStream result = initialSchemaKStream
        .filter(filterNode.getPredicate(), childContextStacker, processingLogContext);

    // Then:
    final Schema resultSchema = result.getSchema();
    assertThat(resultSchema.fields(), hasSize(8));
    assertThat(resultSchema.fields().get(2).name(), is("TEST1.COL0"));
    assertThat(resultSchema.fields().get(2).schema().type(), is(Schema.Type.INT64));
    assertThat(resultSchema.fields().get(3).name(), is("TEST1.COL1"));
    assertThat(resultSchema.fields().get(3).schema().type(), is(Schema.Type.STRING));
    assertThat(resultSchema.fields().get(4).name(), is("TEST1.COL2"));
    assertThat(resultSchema.fields().get(4).schema().type(), is(Schema.Type.STRING));
    assertThat(resultSchema.fields().get(5).name(), is("TEST1.COL3"));
    assertThat(resultSchema.fields().get(5).schema().type(), is(Schema.Type.FLOAT64));
    assertThat(result.getSourceSchemaKStreams().get(0), is(sameInstance(initialSchemaKStream)));
  }
  ...
  public void shouldSelectKey() {
    ...
    final Field newKeyField = initialSchemaKStream
        .getSchema().fields()
        .get(3);

    // When:
    final SchemaKStream rekeyedSchemaKStream = initialSchemaKStream
        .selectKey(newKeyField, true, childContextStacker);
  ...
  public void shouldGroupByKey() {
    ...
    final SchemaKGroupedStream groupedSchemaKStream = initialSchemaKStream.groupBy(
        rowSerde, Collections.singletonList(COL0), childContextStacker);
  ...
  @Test
  public void shouldGroupByMultipleColumns() {
    // Given:
    givenInitialKStreamOf("SELECT col0, col1 FROM test1 WHERE col0 > 100;");

    // When:
    final SchemaKGroupedStream groupedSchemaKStream = initialSchemaKStream.groupBy(
        rowSerde, Arrays.asList(COL1, COL0), childContextStacker);
  ...
  @Test
  public void shouldGroupByMoreComplexExpression() {
    // Given:
    givenInitialKStreamOf("SELECT col0, col1 FROM test1 WHERE col0 > 100;");
    ...
    // When:
    final SchemaKGroupedStream groupedSchemaKStream = initialSchemaKStream
        .groupBy(rowSerde, ImmutableList.of(groupBy), childContextStacker);
  ...
  private PlanNode givenInitialKStreamOf(final String selectQuery) {
    ...
    initialSchemaKStream = new SchemaKStream(logicalPlan.getTheSourceNode().getSchema(), kStream,
        ...
        functionRegistry, queryContext.push("source").getQueryContext());
