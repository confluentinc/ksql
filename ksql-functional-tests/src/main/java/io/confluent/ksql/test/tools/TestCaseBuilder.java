...
public final class TestCaseBuilder {
  ...
  public TestCaseBuilder() {
    this(TestFunctionRegistry.INSTANCE.get());
  }

  public TestCaseBuilder(final FunctionRegistry functionRegistry) {
  ...
  private TestCase createTest(
    ...
    final String testName = buildTestName(testPath, test.name(), explicitFormat);

    try {
      final List<String> statements = buildStatements(test.statements(), explicitFormat);

      final Optional<KsqlExpectedException> ee = test.expectedException()
  ...
  private Map<String, Topic> getTestCaseTopics(
    ...
    statements.stream()
        .map(this::createTopicFromStatement)
    ...
    Streams.concat(test.inputs().stream(), test.outputs().stream())
  ...
  private Topic createTopicFromStatement(final String sql) {
