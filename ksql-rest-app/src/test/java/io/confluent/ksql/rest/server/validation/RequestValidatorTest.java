...
public class RequestValidatorTest {
  ...
  public void shouldThrowExceptionIfValidationFails() {
    // Given:
    givenRequestValidator(
        of(CreateStream.class, statementValidator)
    );
    ...
    final Exception e = assertThrows(
        KsqlException.class,
        () -> validator.validate(serviceContext, statements, of(), "sql")
    );
  ...
  public void shouldThrowIfTooManyPersistentQueries() {
    // Given:
    when(ksqlConfig.getInt(KSQL_ACTIVE_PERSISTENT_QUERY_LIMIT_CONFIG)).thenReturn(1);
  ...
  public void shouldThrowIfServiceContextIsNotSandbox() {
    ...
    givenRequestValidator(of());

    // When:
    final Exception e = assertThrows(
        IllegalArgumentException.class,
        () -> validator.validate(serviceContext, ImmutableList.of(), of(), "sql")
    );
