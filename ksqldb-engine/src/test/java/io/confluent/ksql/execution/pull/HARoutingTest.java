...
public class HARoutingTest {
  ...
  public void shouldCallRouteQuery_success() throws InterruptedException, ExecutionException {
    ...
    when(ksqlClient.makeQueryRequest(eq(node2.location()), any(), any(), any(), any(), any(), Function.identity())).thenAnswer(
      new Answer() {
        private int count = 0;
    ...
    verify(ksqlClient, times(2)).makeQueryRequest(eq(node2.location()), any(), any(), any(), any(), any(), Function.identity());
  ...
  public void shouldCallRouteQuery_twoRound() throws InterruptedException, ExecutionException {
    ...
    CompletableFuture<Void> future = haRouting.handlePullQuery(serviceContext, pullPhysicalPlan,
        statement, routingOptions, pullQueryQueue, disconnect);
    future.get();

    // Then:
    verify(pullPhysicalPlan).execute(eq(ImmutableList.of(location1)), any(), any());
    verify(pullPhysicalPlan).execute(eq(ImmutableList.of(location3)), any(), any());
    verify(ksqlClient, times(3)).makeQueryRequest(eq(node2.location()), any(), any(), any(), any(), any(), Function.identity());
  ...
  public void shouldCallRouteQuery_twoRound_networkError()
    ...
    // Given:
    locate(location2);
    when(ksqlClient.makeQueryRequest(eq(node2.location()), any(), any(), any(), any(), any(), Function.identity()))
        .thenAnswer(i -> {
  ...
  public void shouldCallRouteQuery_allStandbysFail() {
    ...
    verify(pullPhysicalPlan).execute(eq(ImmutableList.of(location1)), any(), any());
    verify(pullPhysicalPlan).execute(eq(ImmutableList.of(location3)), any(), any());
    verify(ksqlClient, atLeast(3)).makeQueryRequest(eq(node2.location()), any(), any(), any(), any(), any(), Function.identity());
  ...
  public void shouldNotRouteToFilteredHost() throws InterruptedException, ExecutionException {
    ...
    when(ksqlClient.makeQueryRequest(any(), any(), any(), any(), any(), any(), Function.identity()))
    ...
    verify(ksqlClient, never())
        .makeQueryRequest(eq(badNode.location()), any(), any(), any(), any(), any(), Function.identity());
