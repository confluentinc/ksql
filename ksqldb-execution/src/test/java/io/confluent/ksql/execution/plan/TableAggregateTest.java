...
public class TableAggregateTest {
  ...
  @Mock
  private ExecutionStep<KGroupedTableHolder> source1;
  @Mock
  private ExecutionStep<KGroupedTableHolder> source2;
  @Mock
  ...
  public void shouldImplementEqualsCorrectly() {
    new EqualsTester()
        .addEqualityGroup(
            new TableAggregate(properties1, source1, formats1, columnRefs1, functionCalls1),
            new TableAggregate(properties1, source1, formats1, columnRefs1, functionCalls1)
        ).addEqualityGroup(
        new TableAggregate(properties2, source1, formats1, columnRefs1, functionCalls1)
    ).addEqualityGroup(
        new TableAggregate(properties1, source2, formats1, columnRefs1, functionCalls1)
    ).addEqualityGroup(
        new TableAggregate(properties1, source1, formats2, columnRefs1, functionCalls1)
    ).addEqualityGroup(
        new TableAggregate(properties1, source1, formats1, columnRefs2, functionCalls1)
    ).addEqualityGroup(
        new TableAggregate(properties1, source1, formats1, columnRefs1, functionCalls2)
    ).testEquals();
  }
}