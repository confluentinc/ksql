...
INSERT INTO `TEST` (DATE) VALUES ('1970-02-10');
INSERT INTO `TEST` (DATE) VALUES ('1970-01-06');
ASSERT VALUES `TEST2` (KSQL_COL_0) VALUES ('1970-01-31');
ASSERT VALUES `TEST2` (KSQL_COL_0) VALUES ('1969-12-27');

--@test: datesub - throws on incorrect type
--@expected.error: io.confluent.ksql.util.KsqlStatementException
--@expected.message: Function 'datesub' does not accept parameters (DATE, INTEGER, DATE).
CREATE STREAM TEST (ID STRING KEY, date DATE) WITH (kafka_topic='test', value_format='DELIMITED');
CREATE STREAM TEST2 AS SELECT id, datesub(date, 5, date) FROM TEST;
--@test: datesub - subtracts negative intervals
...
INSERT INTO `TEST` (ID, date) VALUES (0, '1969-12-27');
ASSERT VALUES `TEST2` (ID, VALUE) VALUES (0, '1970-01-11');
ASSERT VALUES `TEST2` (ID, VALUE) VALUES (0, '1970-01-01');

--@test: datesub - handles null values
