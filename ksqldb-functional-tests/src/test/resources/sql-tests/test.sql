...
ASSERT STREAM foo (id INT KEY, col1 INT) WITH (kafka_topic='foo', value_format='AVRO', value_schema_id=123);
...
--@expected.error: io.confluent.ksql.util.KsqlException
--@expected.message: KAFKA does not support the following configs: [KEY_SCHEMA_ID]
----------------------------------------------------------------------------------------------------
CREATE STREAM foo (id INT KEY, col1 INT) WITH (kafka_topic='foo', key_format='KAFKA', value_format='AVRO', key_schema_id=123);
ASSERT STREAM foo (id INT KEY, col1 INT) WITH (kafka_topic='foo', key_format='KAFKA', value_format='AVRO', key_schema_id=123);