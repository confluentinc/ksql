...
public final class Table {
  ...
  public void print(final HttpServerResponse response) {

    header.forEach(m -> response.write(m).write("\n"));
    ...
    if (columnHeaders.size() > 0) {
      ...
      response.write(String.format(rowFormatString, columnHeaders.toArray()));

      final String separator =
          StringUtils.repeat('-', Math.min(80, separatorLength));
      response.write(separator).write("\n");
      for (final List<String> row : rowValues) {
        response.write(String.format(rowFormatString, row.toArray()));
      }
      response.write(separator).write("\n");
    }

    footer.forEach(m -> response.write(m).write("\n"));
  }
