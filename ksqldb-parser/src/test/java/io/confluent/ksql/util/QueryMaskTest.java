...
public class QueryMaskTest {
  ...
  public void shouldMaskIfNotExistSourceConnector() {
    // Given:
    final UnMaskedStatement query = UnMaskedStatement.of(
        "CREATE SOURCE CONNECTOR IF NOT EXISTS testconnector WITH ("
    ...
    // When
    final MaskedStatement maskedQuery = QueryMask.getMaskedStatement(query);

    // Then
    final MaskedStatement expected = MaskedStatement.of(
        "CREATE SOURCE CONNECTOR IF NOT EXISTS testconnector WITH "
            + "(\"connector.class\"='PostgresSource', "
  ...
  public void shouldMaskSinkConnector() {
    // Given:
    final UnMaskedStatement query = UnMaskedStatement.of(
        "CREATE Sink CONNECTOR `test-connector` WITH ("
            + "    \"connector.class\" = 'PostgresSource', \n"
            + "    'connection.url' = 'jdbc:postgresql://localhost:5432/my.db',\n"
            + "    \"mode\"='bulk',\n"
            + "    \"topic.prefix\"='jdbc-',\n"
  ...
  public void shouldMaskInvalidSinkConnector() {
    ...
    final UnMaskedStatement query = UnMaskedStatement.of(
        "CREATE Sink CONNECTO `test-connector` WITH ("
            + "    \"connector.class\" = 'PostgresSource', \n"
            + "    'connection.url' = 'jdbc:postgresql://localhost:5432/my.db',\n"
            + "    `mode`='bulk',\n"
            + "    \"topic.prefix\"='jdbc-',\n"
  ...
  public void shouldNotMaskInvalidCreateStreamWithoutQuote() {
    // Given:
    // Typo in "WITH" => "WIT"
    final UnMaskedStatement query = UnMaskedStatement.of("CREATE STREAM `stream` WIT ("
        + "    format = 'avro', \n"
    ...
    // Then
    assertThat(maskedQuery.toString(), is(query.toString()));
  }
  ...
  public void shouldMaskValidCreateConnectorWithComment() {
    ...
    // Then
    final MaskedStatement expected = MaskedStatement.of("CREATE SOURCE CONNECTOR `test-connector` WITH "
        + "(\"connector.class\"='PostgresSource', "
        + "'connection.url'='[string]', "
        + "\"mode\"='[string]', "
        + "\"topic.prefix\"='[string]', "
  ...
  public void shouldMaskMultipleValidStatements() {
    // Given:
    // Typo in "WITH" => "WIT"
    final UnMaskedStatement query = UnMaskedStatement.of("CREATE SOURCE CONNECTOR test_connector WITH ("
        + "    \"connector.class\" = 'PostgresSource', \n"
        + "    'connection.url' = 'jdbc:postgresql://localhost:5432/my.db',\n"
        + "    `mode`='bulk',\n"
        + "    \"topic.prefix\"='jdbc-',\n"
    ...
    // Then
    final MaskedStatement expected = MaskedStatement.of("CREATE SOURCE CONNECTOR test_connector WITH "
        + "(\"connector.class\"='PostgresSource', "
        + "'connection.url'='[string]', "
        + "`mode`='[string]', "
        + "\"topic.prefix\"='[string]', "
  ...
  public void shouldMaskMixedValidInvalidStatements() {
    ...
    // Then
    final MaskedStatement expected = MaskedStatement.of("CREATE SOURCE CONNECTOR test_connector WITH (    \"connector.class\" = 'PostgresSource', \n    "
        + "'connection.url'='[string]',\n    "
        + "`mode`='[string]',\n    "
        + "\"topic.prefix\"='[string]',\n    "
        + "\"table.whitelist\"='[string]',\n    "
  ...
  public void shouldMaskInsertStatement() {
    ...
    // Then
    final MaskedStatement expected = MaskedStatement.of("INSERT INTO `FOO` (`KEY_COL`, `COL_A`) VALUES ('[value]', '[value]');");
  ...
  public void shouldMaskFallbackInsertStatement() {
    ...
    // Then
    final MaskedStatement expected = MaskedStatement.of("--this is a comment. \n"
        + "INSERT INTO foo (KEY_COL, COL_A) VALUES"
