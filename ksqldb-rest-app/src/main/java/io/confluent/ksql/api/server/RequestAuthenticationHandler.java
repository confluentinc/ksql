...
public final class RequestAuthenticationHandler {

  private RequestAuthenticationHandler() {}
  ...
  static void setupAuthHandlers(final Server server, final Router router) {
    ...
    if (jaasAuthHandler.isPresent() || authenticationPlugin.isPresent()) {
      router.route().handler(RequestAuthenticationHandler::pauseHandler);
      ...
      router.route().handler(RequestAuthenticationHandler::resumeHandler);
    }
  }
