...
/**
 * Stores information about an execution step plan, needed by the {@link KSPlanBuilder}
 * when converting an execution step plan into a Kafka Streams topology.
 *
 * <p>Today, the information stored includes all source steps and whether each is
 * repartitioned at any point prior to a join. This information is used to determine
 * whether to force materialization on table sources. If a table source is repartitioned
 * downstream prior to a join, then we do not need to materialize the source table as
 * the join will create a state store from the repartition topic rather than the source
 * topic. See https://github.com/confluentinc/ksql/issues/6650 and the use of
 * {@code KSPlanInfo} in {@link SourceBuilder} for more.
 */
public class KSPlanInfo implements PlanInfo {
  ...
  public KSPlanInfo(final ExecutionStep<?> sourceStep) {
  ...
  private KSPlanInfo(final Sources sources) {
  ...
  public KSPlanInfo setIsRepartitionedInPlan() {
    activeSource.ifPresent(sourceInfo -> sourceInfo.isRepartitionedInPlan = true);
    return this;
  }

  public KSPlanInfo merge(final KSPlanInfo other) {
    return new KSPlanInfo(allSources.merge(other.allSources));
  }
