...
public class StreamAggregateBuilderTest {
  ...
  private void givenTimeWindowedAggregate() {
    when(materializedFactory.<GenericKey, WindowStore<Bytes, byte[]>>create(any(), any(), any()))
  ...
  private void givenSessionWindowedAggregate() {
    when(materializedFactory.<GenericKey, SessionStore<Bytes, byte[]>>create(any(), any(), any()))
