...
public class StreamStreamSelfJoinBuilderTest {
  ...
  @Mock
  private KStreamHolder<Struct> self;
  @Mock
  ...
  public void init() {
    ...
    when(left.build(any(), eq(planInfo), eq(true))).thenReturn(
        new KStreamHolder<>(leftKStream, LEFT_SCHEMA, executionKeyFactory));
    when(right.build(any(), eq(planInfo), eq(false))).thenReturn(
        new KStreamHolder<>(rightKStream, RIGHT_SCHEMA, executionKeyFactory));
    when(self.getStream()).thenReturn(leftKStream);
//    when(self.getSchema()).thenReturn(RIGHT_SCHEMA);
//    when(self.getExecutionKeyFactory()).thenReturn(executionKeyFactory);
//
...
//    when(leftKStream.outerJoin(any(KStream.class), any(KsqlValueJoiner.class), any(), any(StreamJoined.class))).thenReturn(resultKStream);
  ...
  public void shouldDoSelfJoin() {
    ...
    verifyNoInteractions(rightKStream);
  ...
  @Test
  public void shouldBuildLeftSourceWithTrueRightWithFalse() {
    // Given:
    givenSelfJoin(L_KEY);
    ...
    // Then:
    verify(left).build(planBuilder, planInfo, true);
    verify(right).build(planBuilder, planInfo, false);
  }
  ...
  private void givenSelfJoin(final ColumnName keyName) {
    join = new StreamStreamSelfJoin<>(
